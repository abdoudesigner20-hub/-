...
class EnhancedBankUI(BoxLayout):
    def __init__(self, **kwargs):
        ...
        self.update_rates_from_internet() 
        Clock.schedule_interval(lambda dt: self.update_rates_from_internet(), 60)
    ...
    def update_rates_from_internet(self):
        try:
            ...
            if data.get("result") == "success":
                self.old_rates = self.rates.copy()
                ...
                for app_code, api_code in mapping.items():
                    if api_code in live_rates: 
                        self.rates[app_code] = live_rates[api_code]
                try:
                    btc_r = requests.get("https://api.coinbase.com/v2/prices/BTC-DZD/spot", timeout=3).json()
                    self.rates["BTC"] = 1 / float(btc_r["data"]["amount"])
                except: pass
...
class BankApp(App):
    def build(self):
        # تهيئة KivMob بمعرف التطبيق الحقيقي
